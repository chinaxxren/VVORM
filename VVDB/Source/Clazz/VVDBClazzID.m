...
- (NSArray *)sqliteColumnsWithAttribute:(VVStoreRuntimeProperty *)attribute {
    VVStoreSQLiteColumnModel *value = [[VVStoreSQLiteColumnModel alloc] init];
    value.columnName = attribute.columnName;
    value.dataTypeName = SQLITE_DATA_TYPE_NONE;

    VVStoreSQLiteColumnModel *attributeType = [[VVStoreSQLiteColumnModel alloc] init];
...
- (NSArray *)storeValuesWithValue:(NSObject *)value attribute:(VVStoreRuntimeProperty *)attribute {
    NSString *attributeType = nil;
    if (value) {
        VVDBClazz *osclazz = [VVDBClazz osclazzWithClazz:[value class]];
